...
class scrabbleBinaryTest {
  ...
  void testToString() {
    var sBins =
        new String[] {
          sieteBin, menosSieteBin, veinteBin, menosVeinteBin, menosDoceBin, cincuentaYTresBin
        };
  ...
  void testAddSubtractMultiplyDivide() {
    ...
    for (int i = 0; i < binarios.length; i++) {
      scrabbleBinary expectedSum =
          new scrabbleInt(RBins[i].toScrInt().getsInt() + binarios[i].toScrInt().getsInt())
              .toScrBin();
      assertEquals(expectedSum, binarios[i].add(RBins[i]));
      scrabbleBinary expectedRes =
          new scrabbleInt(binarios[i].toScrInt().getsInt() - RBins[i].toScrInt().getsInt())
              .toScrBin();
      assertEquals(expectedRes, binarios[i].subtract(RBins[i]));
      scrabbleBinary expectedMul =
          new scrabbleInt(binarios[i].toScrInt().getsInt() * RBins[i].toScrInt().getsInt())
              .toScrBin();
      assertEquals(expectedMul, binarios[i].multiply(RBins[i]));
      scrabbleBinary expectedDiv =
          new scrabbleInt(binarios[i].toScrInt().getsInt() / RBins[i].toScrInt().getsInt())
              .toScrBin();
